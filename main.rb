...
File.read("project_test.txt").each_line{ |file_line|
	...
	puts "Project_name: #{project_name}  #{project_test_name}"
	xml_project_response = teamcity.get_team_city_obj("project", project_name)
	team_city_project_name = teamcity.get_project_name(xml_project_response)

	#Get the list of buildType ids and names (Team City Test Suite names and ids) of the project
	xml_builds_response = teamcity.get_all_builds_obj(xml_project_response,project_test_name)
	xml_test_run_response = teamcity.get_latest_run_obj(xml_builds_response)
	
	#Get the test execution status of the latest run
	test_name = teamcity.get_test_info("name", xml_test_run_response)
	status = teamcity.get_test_info("status",xml_test_run_response)
	test_failed = teamcity.get_test_info("test failed", xml_test_run_response)
	finish_date = teamcity.get_test_info("finish date", xml_test_run_response)
	#Calculate the lapse time since the test last executed
	last_run = utilities.get_run_time_lapse(finish_date,DateTime.now.to_s)
	
	green_since = teamcity.get_last_success_run(xml_builds_response)
